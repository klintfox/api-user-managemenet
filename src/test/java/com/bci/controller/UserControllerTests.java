...
public class UserControllerTests {
	...
	void register_UserSuccessfullyCreated_Returns201() throws Exception {
		// Arrange
		UserDto userDto = new UserDto(); // Suponiendo que UserDto tiene un constructor sin argumentos
	...
	void register_UserBadRequest_Returns400() throws Exception {
		// Arrange
		UserDto userDto = new UserDto(); // Simular un DTO con datos inválidos
	...
	void register_UnauthorizedAccess_Returns401() throws Exception {
		// Arrange
		UserDto userDto = new UserDto(); // Suponiendo que el usuario necesita autenticación
		...
		// Act & Assert
		mockMvc.perform(post("/api/users/register")
				.with(user("admin").password("admin2").roles("USER"))
				.contentType(MediaType.APPLICATION_JSON)
	...
	void register_ResourceConflict_Returns409() throws Exception {
		// Arrange
		UserDto userDto = new UserDto(); // Simular un DTO que causa conflicto
	...
	void register_ResourceNotFound_Returns404() throws Exception {
		// Arrange
		UserDto userDto = new UserDto(); // Simular un DTO que busca un recurso no existente
		...
		mockMvc.perform(post("/api/users/register")
				.with(user("admin").password("admin").roles("USER"))
				.contentType(MediaType.APPLICATION_JSON)
				.content(objectMapper.writeValueAsString(userDto))).andExpect(status().isNotFound())
